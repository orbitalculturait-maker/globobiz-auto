...
function mergeTurno(base = {}, manual = null) {
  ...
  if (manual && typeof manual === 'object') {
    if (Object.prototype.hasOwnProperty.call(manual, 'diurna')) merged.diurna = String(manual.diurna || '').trim() || merged.diurna || 'Leonardo';
...
function getMonthRows(month) {
  ...
  for (const [date, turno] of Object.entries(turni)) {
    ...
    // Se la voce proviene da un override manuale, applichiamo merge completo
    if (turno.fonte === 'manuale') {
      monthMap[date] = mergeTurno(monthMap[date], turno);
      continue;
    }

    // Per i turni provenienti dal file o predefiniti, sovrascrive solo valori non vuoti
    const current = monthMap[date] || { diurna: 'Leonardo', serale: '', fonte: 'vuoto' };
    const updated = { ...current };
    if (turno.diurna) updated.diurna = turno.diurna;
    if (turno.serale) updated.serale = turno.serale;
    if (turno.fonte) updated.fonte = turno.fonte;
    monthMap[date] = updated;
  }
...
function saveMonthRows(month, rows = []) {
  ...
  for (const row of rows) {
    ...
    manualSchedule[row.data] = {
      diurna: String(row.diurna || '').trim() || 'Leonardo',
